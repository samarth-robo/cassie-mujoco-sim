...
double ik_com[3] = {0,0,0};
double ik_lf[3] = {0,0,0};
double ik_rf[3] = {0,0,0};
...
void makeik(int oldstate)
{
    int left_foot_body_id, right_foot_body_id;
    left_foot_body_id = mj_name2id(m, mjOBJ_BODY, "left-foot");
    right_foot_body_id = mj_name2id(m, mjOBJ_BODY, "right-foot");

    for (int i=0; i<3; i++)
    {
        ik_com[i] = d->subtree_com[i];
        ik_lf[i] = d->xpos[3*left_foot_body_id + i];
        ik_rf[i] = d->xpos[3*right_foot_body_id + i];
    }

    mjuiDef defIK[] = 
    {
        {mjITEM_SECTION,   "IK",            oldstate, NULL,                 "AP"},
        {mjITEM_BUTTON,    "Calculate",     2},
        {mjITEM_EDITNUM,   "COM",           2, ik_com,              "3"},
        {mjITEM_EDITNUM,   "Left foot",     2, ik_lf,              "3"},
        {mjITEM_EDITNUM,   "Right foot",    2, ik_rf,              "3"},
        {mjITEM_EDITNUM,   "Translation",   2, d->qpos,              "3"},
        {mjITEM_END}
    };
...
void uiEvent(mjuiState* state)
{
        ...
        state->type==mjEVENT_KEY )
    {
        ...
        else if ( it && it->sectionid==SECT_IK )
        {
            if ( it->itemid==0 )
            {
                std::cout<<"Button clicked"<<std::endl;
                std::cout<<"COM: "<<ik_com[0]<<", "<<ik_com[1]<<", "<<ik_com[2]<<std::endl;
                std::cout<<"LF : "<<ik_lf[0]<<", "<<ik_lf[1]<<", "<<ik_lf[2]<<std::endl;
                std::cout<<"RF : "<<ik_rf[0]<<", "<<ik_rf[1]<<", "<<ik_rf[2]<<std::endl;

                if( cassie_ik(m, d, ik_lf[0], ik_lf[1], ik_lf[2]+0.05,
                        ik_rf[0], ik_rf[1], ik_rf[2], ik_com[0], ik_com[1], ik_com[2], false) )
                {
                    std::cout<<"calculate ik succeed"<<std::endl;
                }
            }
        }
